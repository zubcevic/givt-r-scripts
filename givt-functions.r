...
CollecteKerk <- givt_all %>% filter(str_detect(Toewijzing, var_Collecte1_label)&!str_detect(Toewijzing, var_Collecte2_label)) %>% filter(!is.na(Brutobedrag),!is.na(Aantal.giften)) %>% group_by(period) %>% summarise(totaalKerk=sum(as.numeric(str_replace(Brutobedrag,',','.'))))
CollecteDiaconie <- givt_all %>% filter(str_detect(Toewijzing, var_Collecte2_label)) %>% filter(!is.na(Brutobedrag),!is.na(Aantal.giften)) %>% group_by(period) %>% summarise(totaalDiaconie=sum(as.numeric(str_replace(Brutobedrag,',','.'))))
Collectes_per_Period <- merge(CollecteKerk, CollecteDiaconie, by.x="period",by.y="period")
...
mtext(side = 3, line = 0.25, at = 1, adj = -1.8, mysubtitle)
return(myplot)
}
